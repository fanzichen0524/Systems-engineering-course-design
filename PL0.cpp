...
int gen(enum fct x,int y,int z)
{
	...
	code[cx].l=y;
	code[cx].a=z;
	code[cx].flag=-1;//初始化命令所在行数为-1 ，同时也表指令未被翻译，
	cx++;
	return 0;
}
/*
...
void interpret()
{
	...
	int pi = 0, p_level = -1, pointer = 0;	//p_level记录指令所在层次 
	int array[10];//记录标签号码的数组 
	...
	for(int j=0;j<cx;++j){
		i=code[j];         /*读当前指令*/
		pi=pi+1;
		code[j].flag = pi - 1;	/*该指令已被翻译，记录指令行号*/
		switch(i.f)
		{
			case opr:        /*数字、逻辑运算*/
				if(i.a==0){
	...
	int array_p[10], array_b[10], array_t[10];//分别对应上文p,b,t指针
	array_p[0] = 0;array_b[0] = 0;array_t[0] = 8;
	int p_regs = 8;   /*当前的寄存器（和pl0栈顶相对应）*/
			...
			case opr:        /*数字、逻辑运算*/
				switch(i.a)
				{
					case 0:
						if(i.flag != -1){
							code[p-1].flag = -1;
							p_regs = array_t[p_level];		//回到上一级调用的寄存器
							p_level--;		//回到上一层
							if (p_level >= 0);
							else{
								fprintf(res,"\tj  exit\n");
